...
router.post('/:id/reset-password', async (request, response) => {
  const { id } = request.params
  const providedPassword = typeof request.body?.password === 'string' ? request.body.password.trim() : ''
  const newPassword = providedPassword || `crm-${Math.random().toString(36).slice(2, 8)}`
