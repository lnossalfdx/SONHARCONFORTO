...
router.get('/summary', roleGuard('admin'), async (request, response) => {
  ...
  let salesQuery = supabase
    .from('sales')
    .select('id, value, discount, status, createdAt, payments:sale_payments(method, amount)')
  ...
  let expenseQuery = supabase.from('finance_expenses').select('amount, method')
  if (typeof start === 'string') {
...
router.get('/expenses', roleGuard('admin'), async (request, response) => {
  ...
  let query = supabase
    ...
    .select('id, description, amount, date, method, note, createdBy:createdById(id, name)')
