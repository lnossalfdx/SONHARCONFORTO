...
const normalizeSale = (sale: any): Sale => {
  ...
  payments: Array.isArray(sale.payments)
    ...
    : [],
  note: sale.note ?? '',
  status: normalizedStatus.startsWith('cancel')
    ? 'cancelada'
    : normalizedStatus.startsWith('entreg')
      ? 'entregue'
      : 'pendente',
...
const readSessionCache = <T,>(key: string, ttlSeconds: number): T | null => {
  try {
    const raw = sessionStorage.getItem(key)
    if (!raw) return null
    const parsed = JSON.parse(raw) as CacheEntry<T>
    if (!parsed || typeof parsed.ts !== 'number') return null
    if (Date.now() - parsed.ts > ttlSeconds * 1000) return null
...
const writeSessionCache = <T,>(key: string, data: T) => {
  try {
    ...
    // Ignore cache write failures (quota/disabled storage).
  }
}
...
function App() {
  ...
  const [saleReceiptImages, setSaleReceiptImages] = useState<string[]>([])
  ...
  const [clientsPageItems, setClientsPageItems] = useState<Client[]>([])
  const [clientsPageTotal, setClientsPageTotal] = useState(0)
  const [clientsPageLoading, setClientsPageLoading] = useState(false)
  const [clientsPageError, setClientsPageError] = useState<string | null>(null)
  ...
  const [stockPageItems, setStockPageItems] = useState<StockItem[]>([])
  ...
  const [salesPageItems, setSalesPageItems] = useState<Sale[]>([])
  const [salesPageTotal, setSalesPageTotal] = useState(0)
...
const [expenseSubmitError, setExpenseSubmitError] = useState<string | null>(null)
  ...
  const [userPasswordResetId, setUserPasswordResetId] = useState<string>('')
  ...
  const financeSummaryCacheKey = useMemo(() => {
    ...
    const startKey = financeDateStart || 'all'
    const endKey = financeDateEnd || 'all'
    return `${SUMMARY_CACHE_KEY_PREFIX}:${sessionUserId ?? 'unknown'}:${startKey}:${endKey}:${financePaymentFilter}`
  }, [needsFinanceData, sessionUserId, financeDateStart, financeDateEnd, financePaymentFilter])
  const stockFetchKeyRef = useRef<string | null>(null)
  const stockMovementsFetchKeyRef = useRef<string | null>(null)
  const financeSummaryFetchKeyRef = useRef<string | null>(null)
  const financeExpensesFetchKeyRef = useRef<string | null>(null)
  const monthlyGoalFetchKeyRef = useRef<string | null>(null)
  const assistancesFetchKeyRef = useRef<string | null>(null)
  const salesPageFetchKeyRef = useRef<string | null>(null)
  const clientsPageFetchKeyRef = useRef<string | null>(null)
  const stockPageFetchKeyRef = useRef<string | null>(null)
  ...
  useEffect(() => {
    ...
    const clearTimer = () => {
      if (mobileToolbarTimerRef.current) {
    ...
    const handlePointerDown = (event: PointerEvent) => {
      ...
      const isInsideToolbar = sidebarRef.current ? sidebarRef.current.contains(target) : false
      if (isInsideToolbar) {
        setMobileToolbarCollapsed(false)
      } else {
        setMobileToolbarCollapsed(true)
  ...
  const fetchClientsFromApi = useCallback(async (): Promise<Client[]> => {
    if (!authToken) return []
  ...
  const fetchClientsPageFromApi = useCallback(async () => {
    if (!authToken) return
    setClientsPageLoading(true)
    setClientsPageError(null)
    try {
      const params = new URLSearchParams()
      params.set('paginated', '1')
      params.set('limit', ITEMS_PER_PAGE.toString())
      params.set('offset', ((clientsPage - 1) * ITEMS_PER_PAGE).toString())
      const normalizedSearch = clientSearch.trim()
      if (normalizedSearch) params.set('search', normalizedSearch)
      const startIso = getLocalStartOfDayIso(clientDateStart)
      const endIso = getLocalEndOfDayIso(clientDateEnd)
      ...
      const data = Array.isArray(payload) ? payload : Array.isArray(payload?.data) ? payload.data : []
      const normalized = data.map((client: any) => normalizeClient(client))
      setClientsPageItems(normalized)
      const total = Number(payload?.total)
      const resolvedTotal =
        Number.isFinite(total) && total > 0 ? total : normalized.length ? normalized.length : 0
      setClientsPageTotal(resolvedTotal)
    } catch (error) {
      console.error(error)
      setClientsPageError(error instanceof Error ? error.message : 'Falha ao carregar clientes.')
      setClientsPageItems([])
      setClientsPageTotal(0)
    } finally {
      setClientsPageLoading(false)
    }
  }, [
    authToken,
    clientSearch,
...
useEffect(() => {
  setSalesPage(1)
}, [saleSearch, saleFilter, saleDateStart, saleDateEnd, salePaymentFilter, saleMinValue])

useEffect(() => {
  setClientsPage(1)
}, [clientSearch, clientFilter, clientDateStart, clientDateEnd, clientCityFilter])
...
useEffect(() => {
  ...
  const fetchStockPageFromApi = useCallback(async () => {
    if (!authToken) return
    ...
    try {
      ...
      params.set('paginated', '1')
      params.set('limit', ITEMS_PER_PAGE.toString())
      params.set('offset', ((stockPage - 1) * ITEMS_PER_PAGE).toString())
      const normalizedSearch = stockSearch.trim()
      if (normalizedSearch) params.set('search', normalizedSearch)
      if (stockFilter !== 'all') params.set('filter', stockFilter)
      const minValueNumber = stockMinValue ? Number(stockMinValue) : null
      ...
      const data = Array.isArray(payload) ? payload : Array.isArray(payload?.data) ? payload.data : []
      const normalized = data.map((item: any) => normalizeStockItem(item))
      setStockPageItems(normalized)
      const total = Number(payload?.total)
      const resolvedTotal =
        Number.isFinite(total) && total > 0 ? total : normalized.length ? normalized.length : 0
      setStockPageTotal(resolvedTotal)
    } catch (error) {
      console.error(error)
      setStockPageError(error instanceof Error ? error.message : 'Falha ao carregar estoque.')
      setStockPageItems([])
      setStockPageTotal(0)
    } finally {
  ...
  useEffect(() => {
    ...
    fetchStockFromApi(stockCacheKey)
  }, [authToken, needsStockItems, stockCacheKey, stockItems.length, fetchStockFromApi])
  ...
  const fetchSalesPageFromApi = useCallback(async () => {
    ...
    try {
      ...
      params.set('paginated', '1')
      params.set('limit', ITEMS_PER_PAGE.toString())
      params.set('offset', ((salesPage - 1) * ITEMS_PER_PAGE).toString())
      const normalizedSearch = saleSearch.trim()
      if (normalizedSearch) params.set('search', normalizedSearch)
      if (saleFilter !== 'all') params.set('status', saleFilter)
      const startIso = getLocalStartOfDayIso(saleDateStart)
      const endIso = getLocalEndOfDayIso(saleDateEnd)
      if (startIso) params.set('start', startIso)
      if (endIso) params.set('end', endIso)
      ...
      const payload = await response.json()
      const data = Array.isArray(payload) ? payload : Array.isArray(payload?.data) ? payload.data : []
      const normalized = data.map((sale: any) => normalizeSale(sale))
      setSalesPageItems(normalized)
      const total = Number(payload?.total)
      const resolvedTotal =
        Number.isFinite(total) && total > 0 ? total : normalized.length ? normalized.length : 0
      setSalesPageTotal(resolvedTotal)
    } catch (error) {
      console.error(error)
      setSalesPageError(error instanceof Error ? error.message : 'Falha ao carregar vendas.')
      setSalesPageItems([])
      setSalesPageTotal(0)
    } finally {
  ...
  useEffect(() => {
    if (!authToken || !viewingSleepLab) {
      ...
      setSalesPageTotal(0)
      setSalesPageError(null)
      salesPageFetchKeyRef.current = null
      return
    }
    const fetchKey = [
      authToken,
    ...
    if (salesPageFetchKeyRef.current === fetchKey) return
    salesPageFetchKeyRef.current = fetchKey
    fetchSalesPageFromApi()
  }, [
    authToken,
    viewingSleepLab,
    salesPage,
  ...
  useEffect(() => {
    if (!authToken || activePage !== 'clientes') {
      setClientsPageItems([])
      setClientsPageTotal(0)
      setClientsPageError(null)
      clientsPageFetchKeyRef.current = null
      return
    }
    const fetchKey = [
      authToken,
    ...
    if (clientsPageFetchKeyRef.current === fetchKey) return
    clientsPageFetchKeyRef.current = fetchKey
    fetchClientsPageFromApi()
  }, [
    authToken,
    activePage,
    clientsPage,
  ...
  const fetchFinanceSummaryFromApi = useCallback(async (cacheKey?: string) => {
    if (!authToken || !isAdmin) return
  ...
  useEffect(() => {
    if (!authToken || !isAdmin || !needsFinanceData) {
      financeSummaryFetchKeyRef.current = null
      return
    }
    if (!financeSummaryCacheKey) return
    if (financeSummaryFetchKeyRef.current === financeSummaryCacheKey) return
    financeSummaryFetchKeyRef.current = financeSummaryCacheKey
    const cached = readSessionCache<FinanceSummary>(financeSummaryCacheKey, SUMMARY_CACHE_TTL_SECONDS)
    if (cached) {
    ...
    fetchFinanceSummaryFromApi(financeSummaryCacheKey)
  }, [
    authToken,
    isAdmin,
    needsFinanceData,
  ...
  useEffect(() => {
    if (!authToken || !isAdmin || !needsFinanceData) {
      financeExpensesFetchKeyRef.current = null
      return
    }
    const fetchKey = `${authToken}:${financeDateStart || 'all'}:${financeDateEnd || 'all'}:${financePaymentFilter}`
    if (financeExpensesFetchKeyRef.current === fetchKey) return
    financeExpensesFetchKeyRef.current = fetchKey
  ...
  useEffect(() => {
    if (!authToken || !needsMonthlyGoal) {
      ...
      monthlyGoalFetchKeyRef.current = null
      return
    }
    const fetchKey = `${authToken}:${needsMonthlyGoal}`
    if (monthlyGoalFetchKeyRef.current === fetchKey) return
    monthlyGoalFetchKeyRef.current = fetchKey
    fetchMonthlyGoalFromApi()
  }, [authToken, needsMonthlyGoal, fetchMonthlyGoalFromApi])
  ...
  useEffect(() => {
    if (!authToken) {
      setAssistances([])
      setAssistancesError(null)
      assistancesFetchKeyRef.current = null
      return
    }
    if (!needsAssistancesData) {
      assistancesFetchKeyRef.current = null
      return
    }
    const fetchKey = `${authToken}:${needsAssistancesData}`
    if (assistancesFetchKeyRef.current === fetchKey) return
    assistancesFetchKeyRef.current = fetchKey
...
const focusInventoryPanel = (productId?: string) => {
  ...
  const handleDownloadReceipt = async () => {
    ...
    const receiptNode = receiptContentRef.current
    const sections = Array.from(receiptNode.querySelectorAll('.receipt-copy')) as HTMLElement[]
    ...
    receiptNode.classList.add('receipt-export-desktop')
    try {
  ...
  const handlePaymentReceiptUpload = async (saleId: string, event: ChangeEvent<HTMLInputElement>) => {
    ...
    const images = results.filter((result): result is string => typeof result === 'string' && result.startsWith('data:image/'))
    if (!images.length) {
      event.target.value = ''
      return
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorBody?.message ?? 'Não foi possível salvar o comprovante.')
      }
      const payload = await response.json()
      const mergedImages = normalizeReceiptImages(payload?.images ?? payload?.imageData).length
        ? normalizeReceiptImages(payload?.images ?? payload?.imageData)
        : [...saleReceiptImages, ...images]
      setSaleReceiptImages(mergedImages)
    } catch (error) {
      console.error(error)
    } finally {
  ...
  const handleRemovePaymentReceipt = async (saleId: string) => {
    try {
      ...
      if (!response.ok) {
        const errorBody = await response.json().catch(() => null)
        throw new Error(errorBody?.message ?? 'Não foi possível remover o comprovante.')
      }
  ...
  const handleSearchNavigate = (result: SearchResult) => {
    ...
    if (result.type === 'Cliente') {
      ...
      if (result.payload) setClientSearch(result.payload)
      return
    }

    if (result.type === 'Venda') {
      setActivePage('sleepLab')
      const sale = sales.find((item) => item.id === result.id)
      if (sale) {
        void openSaleModal(sale)
        return
      }
      setSaleSearch(result.id)
      return
    }

    if (result.type === 'Estoque') {
      setActivePage('estoque')
      const product = stockItems.find((item) => item.id === result.id)
      if (product) {
        openEditProductModal(product)
        return
      }
      if (result.payload) setStockSearch(result.payload)
      return
    }

    if (result.type === 'Movimento') {
      setActivePage('estoque')
      if (result.payload) setStockSearch(result.payload)
      return
    }
  ...
  const ensureSaleDependencies = async () => {
    const needsClients = clients.length === 0
    const needsStock = stockItems.length === 0
    if (!needsClients && !needsStock) {
      return { resolvedClients: clients, resolvedStockItems: stockItems }
    }
    setSaleModalLoading(true)
    try {
      const [resolvedClients, resolvedStockItems] = await Promise.all([
        needsClients ? fetchClientsFromApi() : Promise.resolve(clients),
        needsStock ? fetchStockFromApi(stockCacheKey) : Promise.resolve(stockItems),
      ])
      return { resolvedClients, resolvedStockItems }
    } finally {
  ...
  const openSaleModal = async (saleToEdit?: Sale) => {
    if (saleToEdit) {
      ...
      const { resolvedClients, resolvedStockItems } = await ensureSaleDependencies()
      const saleBackendId = saleToEdit.backendId
      if (saleBackendId) {
        setSaleReceiptLoading(true)
        try {
          ...
          if (response.ok) {
            const data = await response.json()
            setSaleReceiptImages(normalizeReceiptImages(data?.receiptImage))
          } else {
            setSaleReceiptImages([])
          }
        } catch (error) {
  ...
  const handleRegisterSale = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const normalizedDeliveryDate = normalizeDateToIso(saleForm.deliveryDate)
    const payload = {
      ...
      deliveryDate: normalizedDeliveryDate ? normalizedDeliveryDate : undefined,
    }
  ...
  const handleApproveSale = async (sale: Sale) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        const message =
          typeof data?.message === 'string' ? data.message : 'Não foi possível aprovar este pedido.'
        throw new Error(message)
      }
      const salePayload = data?.sale ?? data
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/finance/expenses`, {
        ...
        body: JSON.stringify({
          description,
          amount: amountNumber,
          date: getLocalStartOfDayIso(expenseForm.date),
      ...
      await Promise.all([fetchFinanceExpensesFromApi(), fetchFinanceSummaryFromApi(financeSummaryCacheKey)])
    } catch (error) {
      console.error(error)
  ...
  const handleSaveMonthlyGoal = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (payload) {
        setMonthlyGoal(payload)
        setMonthlyGoalFormValue(payload.target ?? monthlyGoalFormValue)
      } else {
  ...
  const handleResetUserPassword = async (userId: string, customPassword?: string) => {
    if (!isAdmin) return
    setUserActionLoading(true)
    setUserActionError(null)
    setUserInviteTempPassword(null)
    ...
    try {
      ...
      if (!response.ok) {
        const errorBody = await response.json().catch(() => null)
        throw new Error(errorBody?.message ?? 'Não foi possível redefinir a senha.')
      }
      const data = await response.json()
      ...
      setUserManagerNotice(
        target
          ? `Senha atualizada para ${target.name}.`
          : 'Senha redefinida.',
      )
    } catch (error) {
      console.error(error)
  ...
  const handleCustomPasswordReset = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault()
    if (!isAdmin) return
    if (!userPasswordResetId) {
      setUserActionError('Selecione um usuário para redefinir a senha.')
      return
    }
    if (userPasswordResetValue.trim().length < 6) {
      setUserActionError('A senha deve ter pelo menos 6 caracteres.')
      return
    }
    await handleResetUserPassword(userPasswordResetId, userPasswordResetValue.trim())
    ...
    return (
      <div className={`dashboard-grid${searchFocused ? ' search-mode' : ''}`}>
        <div className={`global-search${searchFocused ? ' active' : ''}`}>
          {isMobileViewport && (
            <button type="button" className="mobile-profile-button" onClick={() => setProfileModalOpen(true)}>
  ...
  const renderLogin = () => (
    <div className="login-page">
      <div className="login-backdrop">
        ...
        <span className="login-particles layer-one" />
        <span className="login-particles layer-two" />
      </div>
  ...
  const renderClients = () => {
    ...
    const pagedClients =
      clientsPageItems.length > ITEMS_PER_PAGE
        ? clientsPageItems.slice((clientsPage - 1) * ITEMS_PER_PAGE, clientsPage * ITEMS_PER_PAGE)
        : clientsPageItems
      ...
      <div className="page-stack">
        ...
        <section className="panel">
          <div className="section-head">
            ...
            <span className="chip ghost">{clientsPageTotal} encontrados</span>
          </div>
          <div className="client-list">
            {clientsPageLoading && <p className="empty-state">Carregando clientes...</p>}
            {!clientsPageLoading && clientsPageError && <p className="empty-state">{clientsPageError}</p>}
            {!clientsPageLoading && !clientsPageError && pagedClients.length === 0 && (
              <p className="empty-state">Nenhum cliente encontrado com os filtros atuais.</p>
            )}
            {!clientsPageLoading && !clientsPageError && pagedClients.map((client) => {
          ...
          {!clientsPageLoading &&
            !clientsPageError &&
            renderPagination(clientsPageTotal, clientsPage, setClientsPage)}
        </section>
      </div>
    );
  };
  ...
  const renderPagination = (
    ...
    const pageLabel = (page: number) => `Página ${page}`
    return (
      ...
      <div className="page-stack sales-page">
        ...
        <section className="panel sales-panel">
          ...
          <div className="sales-list">
            ...
            {!salesPageLoading && !salesPageError && pagedSales.length === 0 && (
                ...
                return (
                  <div className={`sale-card ${sale.status}`} key={sale.id}>
                    ...
                    <div className="sale-card-actions">
                      ...
                      {canManageThisSale && (
                        <button type="button" className="ghost" onClick={() => void openSaleModal(sale)}>
                          Editar pedido
                        </button>
                      )}
          ...
          {!salesPageLoading &&
            !salesPageError &&
            renderPagination(salesPageTotal, salesPage, setSalesPage)}
        </section>
      ...
      <div className="page-stack stock-page">
        ...
        <section className="panel stock-hero">
          ...
          <div className="stock-grid">
            ...
            {!stockPageLoading && stockPageError && <p className="empty-state">{stockPageError}</p>}
            {!stockPageLoading && !stockPageError && pagedStock.length === 0 && (
  ...
  return (
    ...
    <div className="page">
      <div className={`layout ${collapsed ? 'is-collapsed' : ''}`}>
        <aside
          ref={sidebarRef}
          className={`sidebar ${collapsed ? 'collapsed' : ''}${mobileToolbarCollapsed ? ' mobile-collapsed' : ''}`}
          ...
          <nav className="nav">
            {visibleNavItems.map((item) => (
              <button
                ...
                onClick={() => {
                  if (isMobileViewport && mobileToolbarCollapsed) {
                    setMobileToolbarCollapsed(false)
                    return
                  }
      ...
      {receiptPreviewOpen && receiptPreviewUrl && (
        <div className="modal-backdrop" onClick={() => setReceiptPreviewOpen(false)}>
          <div className="modal receipt-preview-modal" onClick={(event) => event.stopPropagation()}>
            <div className="section-head">
              <div>
                <p className="eyebrow">Comprovante</p>
                <h2>Visualização</h2>
              </div>
              <button className="text-button" onClick={() => setReceiptPreviewOpen(false)}>
                Fechar
              </button>
            </div>
            <div className="receipt-preview-body">
              <img src={receiptPreviewUrl} alt="Comprovante de pagamento" />
            </div>
            <div className="modal-actions">
              <a className="primary" href={receiptPreviewUrl} download="comprovante.png">
                Baixar imagem
              </a>
            </div>
          </div>
      ...
      {userManagerOpen && isAdmin && (
        <div className="modal-backdrop" onClick={() => setUserManagerOpen(false)}>
          <div className="modal user-modal" onClick={(event) => event.stopPropagation()}>
            ...
            <form className="user-form" onSubmit={handleCustomPasswordReset}>
              <p className="form-title">Redefinir senha manualmente</p>
              <div className="user-form-grid">
                <label>
                  Usuário
                  <select
                    value={userPasswordResetId}
                    onChange={(event) => setUserPasswordResetId(event.target.value)}
                    disabled={userActionLoading || users.length === 0}
                  >
                ...
                <label>
                  ...
                  <input
                    ...
                    onChange={(event) => setUserPasswordResetValue(event.target.value)}
              ...
              <div className="modal-actions">
                <button className="primary" type="submit" disabled={userActionLoading || users.length === 0}>
                  {userActionLoading ? 'Processando...' : 'Atualizar senha'}
                </button>
              </div>
            </form>
